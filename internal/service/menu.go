...
// GetMenus 获取菜单列表
func (m Menu) GetMenus() ([]*model.Menu, error) {
	var menus []*model.Menu
	err := mysql.DB.Order("sort").Find(&menus).Error
	return menus, err
}

// GetMenuTree 获取菜单树
func (m Menu) GetMenuTree() ([]*model.Menu, error) {
	var menus []*model.Menu
	err := mysql.DB.Order("sort").Find(&menus).Error
	// parentId为0的是根菜单
...
func (m Menu) CreateMenu(menu *model.Menu) error {
	err := mysql.DB.Create(menu).Error
	return err
}

// UpdateMenuById 更新菜单
func (m Menu) UpdateMenuById(menuId uint, menu *model.Menu) error {
	err := mysql.DB.Model(menu).Where("id = ?", menuId).Updates(menu).Error
	return err
}
...
func (m Menu) BatchDeleteMenuByIds(menuIds []uint) error {
	var menus []*model.Menu
	err := mysql.DB.Where("id IN (?)", menuIds).Find(&menus).Error
	if err != nil {
		return err
	}
	err = mysql.DB.Select("Roles").Unscoped().Delete(&menus).Error
	return err
}

// GetUserMenusByUserId 根据用户ID获取用户的权限(可访问)菜单列表
func (m Menu) GetUserMenusByUserId(userId uint) ([]*model.Menu, error) {
	// 获取用户
	var user model.User
	err := mysql.DB.Where("id = ?", userId).Preload("Roles").First(&user).Error
	if err != nil {
		return nil, err
	}
	// 获取角色
	...
	for _, role := range roles {
		...
		err := mysql.DB.Where("id = ?", role.ID).Preload("Menus",
			// 修复排序
...
func (m Menu) GetUserMenuTreeByUserId(userId uint) ([]*model.Menu, error) {

...
type ApiService interface {
	GetApis(req *requ.ApiListRequest) ([]*model.Api, int64, error) // 获取接口列表
...
// GetApis 获取接口列表
func (a Api) GetApis(req *requ.ApiListRequest) ([]*model.Api, int64, error) {
	var list []*model.Api
	db := mysql.DB.Model(&model.Api{}).Order("created_at DESC")
...
func (a Api) GetApisById(apiIds []uint) ([]*model.Api, error) {
	var apis []*model.Api
	err := mysql.DB.Where("id IN (?)", apiIds).Find(&apis).Error
	return apis, err
}
...
func (a Api) GetApiTree() ([]*repo.ApiTreeResp, error) {
	...
	err := mysql.DB.Order("category").Order("created_at").Find(&apiList).Error
	// 获取所有的分类
...
// CreateApi 创建接口
func (a Api) CreateApi(api *model.Api) error {
	err := mysql.DB.Create(api).Error
	return err
}

// UpdateApiById 更新接口
func (a Api) UpdateApiById(apiId uint, api *model.Api) error {
	// 根据id获取接口信息
	var oldApi model.Api
	err := mysql.DB.First(&oldApi, apiId).Error
	if err != nil {
		return errors.New("根据接口ID获取接口信息失败")
	}
	err = mysql.DB.Model(api).Where("id = ?", apiId).Updates(api).Error
	if err != nil {
		return err
	}
	// 更新了method和path就更新casbin中policy
	if oldApi.Path != api.Path || oldApi.Method != api.Method {
		policies := casbin.CasbinEnforcer.GetFilteredPolicy(1, oldApi.Path, oldApi.Method)
		// 接口在casbin的policy中存在才进行操作
		if len(policies) > 0 {
			// 先删除
			isRemoved, _ := casbin.CasbinEnforcer.RemovePolicies(policies)
			if !isRemoved {
			...
			// 新增
			isAdded, _ := casbin.CasbinEnforcer.AddPolicies(policies)
			if !isAdded {
				return errors.New("更新权限接口失败")
			}
			// 加载policy
...
func (a Api) BatchDeleteApiByIds(apiIds []uint) error {
	...
	err = mysql.DB.Where("id IN (?)", apiIds).Unscoped().Delete(&model.Api{}).Error
	// 如果删除成功，删除casbin中policy
	if err == nil {
		for _, api := range apis {
			policies := casbin.CasbinEnforcer.GetFilteredPolicy(1, api.Path, api.Method)
			if len(policies) > 0 {
				isRemoved, _ := casbin.CasbinEnforcer.RemovePolicies(policies)
				if !isRemoved {
...
func (a Api) GetApiDescByPath(path string, method string) (string, error) {
	var api model.Api
	err := mysql.DB.Where("path = ?", path).Where("method = ?", method).First(&api).Error
	return api.Desc, err
}

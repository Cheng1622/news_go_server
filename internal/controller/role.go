...
func (rs RoleApiService) GetRoles(c *gin.Context) {
	...
	if err != nil {
		// 错误返回
		response.Error(c, code.ServerErr, "获取角色列表失败: "+err.Error())
		return
	}
	// 成功返回
...
func (rs RoleApiService) CreateRole(c *gin.Context) {
	var req requ.CreateRoleRequest
	// 参数绑定
	if err := c.ShouldBind(&req); err != nil {
		// 参数校验
		validator.HandleValidatorError(c, err)
	...
	if err != nil {
		// 错误返回
		response.Error(c, code.ServerErr, "获取当前用户最高角色等级失败: "+err.Error())
		return
	}
	...
	if sort >= req.Sort {
		// 错误返回
		response.Error(c, code.ServerErr, "不能创建比自己等级高或相同等级的角色")
		return
	}
	...
	if err != nil {
		// 错误返回
		response.Error(c, code.ServerErr, "创建角色失败: "+err.Error())
		return
	}
	// 成功返回
...
func (rs RoleApiService) UpdateRoleById(c *gin.Context) {
	...
	if roleId <= 0 {
		// 错误返回
		response.Error(c, code.ServerErr, "角色ID不正确")
		return
	}
	...
	if len(roles) == 0 {
		// 错误返回
		response.Error(c, code.ServerErr, "未获取到角色信息")
		return
	}
	...
	if err != nil {
		// 错误返回
		response.Error(c, code.ServerErr, "更新角色失败: "+err.Error())
		return
	}
	...
	if req.Keyword != roles[0].Keyword {
		// 获取policy
		rolePolicies := casbin.CasbinEnforcer.GetFilteredPolicy(0, roles[0].Keyword)
		if len(rolePolicies) == 0 {
			// 错误返回
			response.Error(c, code.ServerErr, "更新角色成功")
			return
		}
		...
		isAdded, _ := casbin.CasbinEnforcer.AddPolicies(rolePolicies)
		if !isAdded {
			// 错误返回
			response.Error(c, code.ServerErr, "更新角色成功，但角色关键字关联的权限接口更新失败")
			return
		}
		isRemoved, _ := casbin.CasbinEnforcer.RemovePolicies(rolePoliciesCopy)
		if !isRemoved {
			// 错误返回
			response.Error(c, code.ServerErr, "更新角色成功，但角色关键字关联的权限接口更新失败")
			return
		...
		if err != nil {
			// 错误返回
			response.Error(c, code.ServerErr, "更新角色成功，但角色关键字关联角色的权限接口策略加载失败")
			return
		}
...
func (rs RoleApiService) GetRoleMenusById(c *gin.Context) {
	...
	if err != nil {
		// 错误返回
		response.Error(c, code.ServerErr, "获取角色的权限菜单失败: "+err.Error())
		return
	}
	// 成功返回
	response.Success(c, code.SUCCESS, gin.H{
...
func (rs RoleApiService) UpdateRoleMenusById(c *gin.Context) {
	var req requ.UpdateRoleMenusRequest
	// 参数绑定
	if err := c.ShouldBind(&req); err != nil {
		// 参数校验
		validator.HandleValidatorError(c, err)
	...
	if minSort != 1 {
		if minSort >= roles[0].Sort {
			// 错误返回
			response.Error(c, code.ServerErr, "不能更新比自己角色等级高或相等角色的权限菜单")
			return
		}
	}
	...
	if err != nil {
		// 错误返回
		response.Error(c, code.ServerErr, "获取当前用户的可访问菜单列表失败: "+err.Error())
		return
	}
	...
	if minSort != 1 {
		for _, id := range menuIds {
			if !funk.Contains(ctxUserMenusIds, id) {
				// 错误返回
				response.Error(c, code.ServerErr, fmt.Sprintf("无权设置ID为%d的菜单", id))
				return
			}
		}
		...
		if err != nil {
			// 错误返回
			response.Error(c, code.ServerErr, "获取菜单列表失败: "+err.Error())
			return
		}
	...
	if err != nil {
		// 错误返回
		response.Error(c, code.ServerErr, "更新角色的权限菜单失败: "+err.Error())
		return
	}
	// 成功返回
	response.Success(c, code.SUCCESS, nil)
...
func (rs RoleApiService) UpdateRoleApisById(c *gin.Context) {
	var req requ.UpdateRoleApisRequest
	// 参数绑定
	if err := c.ShouldBind(&req); err != nil {
		// 参数校验
		validator.HandleValidatorError(c, err)
	...
	if minSort != 1 {
		if minSort >= roles[0].Sort {
			// 错误返回
			response.Error(c, code.ServerErr, "不能更新比自己角色等级高或相等角色的权限接口")
			return
		}
	}
	...
	for _, role := range ctxRoles {
		policy := casbin.CasbinEnforcer.GetFilteredPolicy(0, role.Keyword)
	...
	if err != nil {
		// 错误返回
		response.Error(c, code.ServerErr, "根据接口ID获取接口信息失败")
		return
	}
	// 生成前端想要设置的角色policies
	...
	if minSort != 1 {
		for _, reqPolicy := range reqRolePolicies {
			if !funk.Contains(ctxRolesPolicies, reqPolicy) {
				// 错误返回
				response.Error(c, code.ServerErr, fmt.Sprintf("无权设置路径为%s,请求方式为%s的接口", reqPolicy[1], reqPolicy[2]))
				return
			}
		}
	}
...
func (rs RoleApiService) BatchDeleteRoleByIds(c *gin.Context) {
	var req requ.DeleteRoleRequest
	// 参数绑定
	if err := c.ShouldBind(&req); err != nil {
		// 参数校验
		validator.HandleValidatorError(c, err)
	...
	if err != nil {
		// 错误返回
		response.Error(c, code.ServerErr, "获取角色信息失败: "+err.Error())
		return
	}
	if len(roles) == 0 {
		// 错误返回
	...
	for _, role := range roles {
		if minSort >= role.Sort {
			// 错误返回
			response.Error(c, code.ServerErr, "不能删除比自己角色等级高或相等的角色")
			return
		}
	}
	...
	if err != nil {
		// 错误返回
		response.Error(c, code.ServerErr, "删除角色失败")
		return
